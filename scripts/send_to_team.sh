...
send_to_window() {
    ...
    local message=$2
    # Send the message
    tmux send-keys -t chatgpt-dev-team:$window "$message"
    # ALWAYS send Enter key separately to ensure it gets through
    sleep 0.1
    tmux send-keys -t chatgpt-dev-team:$window C-m
    # Double-check with another Enter after a brief pause
    sleep 0.2
    tmux send-keys -t chatgpt-dev-team:$window C-m
}
